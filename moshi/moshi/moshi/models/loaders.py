...
def get_moshi_lm(
    ...
    if filename is not None:
        # Check if TTT is enabled - if so, we'll have new parameters not in base checkpoint
        has_ttt = lm_kwargs.get('ttt_config', {}).get('enabled', False)
        ...
        if _is_safetensors(filename):
            ...
            for key, value in state.items():
                ...
                state[key] = value
            # Use strict=False when TTT is enabled (new params won't be in base model)
            result = model.load_state_dict(state, assign=True, strict=not has_ttt)
            if has_ttt and result.missing_keys:
                ttt_missing = [k for k in result.missing_keys if 'w_down' in k or 'target_generator' in k]
                if ttt_missing:
                    print(f"[TTT] Initializing {len(ttt_missing)} TTT parameters (not in base checkpoint)")

        else:
            pkg = torch.load(filename, "cpu",)
            model.load_state_dict(pkg["fsdp_best_state"]["model"], assign=True, strict=not has_ttt)

        # Initialize TTT pretrained weight buffers after loading checkpoint
        ttt_layers_found = []
        for idx, layer in enumerate(model.transformer.layers):
            ...
            if mlp and hasattr(mlp, 'ttt_enabled') and mlp.ttt_enabled:
                # w_down might still be on meta device - need to initialize it
                ...
                # Initialize target_generator parameters if on meta device
                if hasattr(mlp, 'target_generator'):
                    tg = mlp.target_generator
                    # Check conv1d
...
def get_lora_moshi(
    ...
    if lora_weights is not None:
        ...
        # Count TTT weights in checkpoint
        ttt_keys = [k for k in lora_state_dict.keys() if 'target_generator' in k]
        if ttt_keys:
            print(f"[TTT] Found {len(ttt_keys)} TTT weights in checkpoint:")
            for key in ttt_keys:
                print(f"  - {key}: shape {lora_state_dict[key].shape}")
        ...
        if res.unexpected_keys:
            # Filter out TTT-specific keys (target_generator weights)
            non_ttt_unexpected = [k for k in res.unexpected_keys 
                                  if 'target_generator' not in k]
            if non_ttt_unexpected:
                raise RuntimeError(
                    f"unexpected_keys in the lora weights: {non_ttt_unexpected}"
                )
