...
try:
    # optional import: ttt module is new and may not be present in older installs
...
class StreamingTransformerLayer(StreamingModule[_LayerState]):
    ...
    def __init__(
        ...
        if gating == "none":
            ...
            self.linear1 = None
            self.linear2 = None
            # decide whether this layer should use TTT gating
            use_ttt = False
            ...
            if weights_per_step:
                ...
                # we don't support weights_per_step + ttt for now
                if use_ttt:
                    # fall back to standard gating list (TTT not supported for per-step)
                    use_ttt = False
                ...
                if use_ttt:
                    # create TTT enabled gating
                    assert TTTGating is not None, "TTTGating implementation missing"
                    # TTTGating expects activation then dim,dim_feedforward,ttt_config
                    self.gating = TTTGating(activation, d_model, dim_feedforward, ttt_config, **factory_kwargs)
                    self.use_ttt = True
                else:
    ...
    def _ff_block(self, x: torch.Tensor, token_embeddings: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if self.gating is None:
            ...
            if self.weights_per_step:
                ...
                # if TTT is enabled for this layer and we received token embeddings,
                # forward them to the gating module which may perform TTT updates.
    ...
    def forward(self, x: torch.Tensor, cross_attention_src: torch.Tensor | None = None, token_embeddings: torch.Tensor | None = None):
...
class StreamingTransformer(StreamingModule[_TransformerState]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Extract ttt_config once before loop
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs):
        ...
        for layer in self.layers:
            if self.checkpointing:
                ...
                # forward token_embeddings through kwargs if present
